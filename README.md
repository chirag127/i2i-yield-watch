...
- **Select + rank:** loans with rate **strictly > `AUTOINVEST_MIN_RATE_PCT`** (default **100**) **AND** credit score **>= `AUTOINVEST_MIN_CREDIT_SCORE`** (default **750** — a missing credit score is imputed 750 and *passes* the gate, never treated as 0), ranked rate desc then `bloan_cibil_score` desc.
...
| `AUTOINVEST_MIN_CREDIT_SCORE` | `750` | **Credit gate** — skip loans with score **<** this (no-score loans are imputed 750 and pass) |
