...
- **Ranking policy (shared by notify sort + auto-invest select)** — order of importance: **1) RETURN (interest rate)**, **2) borrower CREDIT SCORE**, **3) loan TENURE** (longer = better; locks the high rate in longer), then income/amount as minor tie-breaks. A borrower with **NO credit score is ranked AS IF the score were 720** (`config.NO_CREDIT_IMPUTED_SCORE` — passes the 700 gate, ranks as *High Risk / High Uncertainty* below any real 750+ score) but is **flagged "no credit score" in the notification**. `scorer.py` owns the weighted score + `sort_loans` (notify path); `invest.select` owns the auto-invest candidate ranking — both apply this same policy. Tenure parsing = `util.tenure_months` ("6 Months"/"1 Year"/int → months).
